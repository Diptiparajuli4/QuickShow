...
const ListShows = () => {
  ...
  /*  Handles: populated object, raw ObjectId, nested screen/audi,       */
  /*           flat fallback fields, and DB lookup by id                 */
  /* ------------------------------------------------------------------ */
  const resolveTheater = (show) => {
    ...
    const normalize = (t) => {
      ...
      // If it's just an id string -> look it up in the theaters collection
      if (typeof t === "string") {
        const found = byId(t);
    ...
    // Flat fields stored directly on the show document
    if (show.theaterName) {
      return {
