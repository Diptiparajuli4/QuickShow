...
const formatShowDateTime = (dt) => {
    try {
        const d = new Date(dt);
        if (isNaN(d.getTime())) return { date: String(dt), time: "" };
        return {
...
const Home = () => {
    ...
    const fetchAllMovies = useCallback(async () => {
        try {
            const response = await axios.get(`${BACKEND_URL}/show/all`);
            ...
            shows.forEach((show) => {
                ...
                if (movie && typeof movie === "object" && movie._id) {
                    if (!movieMap.has(String(movie._id))) {
    ...
    useAutoRefresh(fetchAllMovies, []);

    // =================================================
    // FETCH MOVIES FOR THEATER
    // =================================================
    const fetchMoviesForTheater = useCallback(async (theater) => {
        ...
        try {
            ...
            // 1. Get movie IDs from the theater document
            let movieIds = Array.isArray(theater.movies)
                ? theater.movies.map(String)
                : [];
            ...
            try {
                ...
                const fresh = res.data?.theater || res.data?.data;
                if (
                    fresh &&
                    Array.isArray(fresh.movies) &&
                    fresh.movies.length > 0
                ) {
                    movieIds = fresh.movies.map(String);
                    console.log(
            ...
            // 2. Fetch all shows, keep only ACTIVE shows for this theater
            const showsRes = await axios.get(`${BACKEND_URL}/show/all`);
            const allShows = Array.isArray(showsRes.data?.shows)
                ? showsRes.data.shows
                : [];
            ...
            const activeShowsAtTheater = allShows.filter((show) => {
                const tid = String(
                    show.theaterId && typeof show.theaterId === "object"
                        ? show.theaterId._id
                        : show.theaterId || show.theater || ""
                );

                if (tid !== String(theater._id)) return false;
            ...
            console.log(
                "Active shows at this theater:",
                activeShowsAtTheater
            );

            // 3. Group active shows by movieId
            const showsByMovie = new Map();

            activeShowsAtTheater.forEach((show) => {
                const m = show.movie;
                const movieId =
                    m && typeof m === "object"
                        ? String(m._id)
                ...
                if (!showsByMovie.has(movieId)) {
                    showsByMovie.set(movieId, {
                        populatedMovie:
                            m && typeof m === "object" ? m : null,
                        shows: [],
                    });
                }
                showsByMovie.get(movieId).shows.push(show);
            });

            // 4. Decide which movies to show
            let targetIds;

            if (movieIds.length > 0) {
                targetIds = movieIds.filter((id) =>
                    showsByMovie.has(id)
                );
            } else {
                targetIds = Array.from(showsByMovie.keys());
            }

            console.log("Movies to display:", targetIds);

            // 5. Build final list, fetching movie details if not populated
            const finalMovies = [];

            for (const movieId of targetIds) {
                const entry = showsByMovie.get(movieId);
                if (!entry) continue;

                let movie = entry.populatedMovie;
                ...
                if (!movie || !movie.title) {
                    try {
                        ...
                        movie = { _id: movieId, title: "Unknown Movie" };
                    }
                }
                ...
                // Price = cheapest active show for this movie
                const prices = entry.shows
                    .map((s) => Number(s.showPrice || 0))
                    .filter((n) => n > 0);
                const price = prices.length > 0 ? Math.min(...prices) : 0;

                // Show times from active shows only
                const showDateTimes = entry.shows
                    .map((s) => ({
                        ...formatShowDateTime(s.showDateTime),
                        raw: s.showDateTime,
                    }))
                    .sort(
                        (a, b) =>
                            new Date(a.raw) - new Date(b.raw)
                    );
            ...
            // Sort by earliest upcoming show
            finalMovies.sort((a, b) => {
                const aT = a._shows?.[0]?.showDateTime;
                const bT = b._shows?.[0]?.showDateTime;
                return new Date(aT) - new Date(bT);
            });

            setAvailableMovies(finalMovies);
        } catch (error) {
            console.error(
    ...
    const handleTheaterClick = async (theater) => {
        ...
        await fetchMoviesForTheater(theater);
    };
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                try {
                    ...
                    const backendTheaters = Array.isArray(
                        response.data?.theaters
                    )
                        ? response.data.theaters
                        : [];

                    const theatersWithDistance = backendTheaters
                        .map((theater) => {
                            let theaterLat = Number(theater.latitude);
                            let theaterLng = Number(theater.longitude);
                            ...
                            if (
                                ...
                                theaterLng = Number(
                                    theater.location.coordinates[0]
                                );
                                theaterLat = Number(
                                    theater.location.coordinates[1]
                                );
                            }
                        ...
                        .filter(
                            (t) =>
                                ...
                                t.distance <= MAX_NEARBY_DISTANCE_KM
                        )
                        ...
                        <div>
                            ...
                            <p className="text-gray-400 text-sm mt-1">
                                {hasSearched && theaters.length === 0
                                    ? `No theaters found within ${MAX_NEARBY_DISTANCE_KM} km.`
                    ...
                    {locationError && !showSettingsModal && (
                        <div className="mt-4 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                            <p className="text-yellow-400 text-sm">
                                ⚠️ Location access is required. Please
                        ...
                        <div className="mt-6">
                            {theaters.length > 0 ? (
                                ...
                                <p className="text-gray-400 text-sm">
                                    No theaters found within{" "}
                                    {MAX_NEARBY_DISTANCE_KM} km of your
                                    location.
                                </p>
                            )}
                        </div>
                        ...
                        {loadingMovies ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                                {availableMovies.map((movie) => (
                                    <MovieCard
                                        ...
                                        showDateTimes={movie.showDateTimes}
                                    />
                                ))}
                            </div>
                        ) : (
                            <div className="py-10 text-center">
                                <p className="text-gray-400">
                                    No movies are currently available at
                                    this theater.
                                </p>
                ...
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 px-4">
                    <div className="bg-gray-900 border border-gray-700 rounded-2xl max-w-md w-full p-6 shadow-2xl">
                        ...
                        <p className="text-gray-300 text-sm mb-4">
                            To find nearby theaters, please enable location
                        ...
                        <p className="text-xs text-gray-500 mt-4 text-center">
                            After enabling location, click{" "}
                            <strong>Retry</strong> to search again.
                        </p>
                    </div>
                </div>
            )}
