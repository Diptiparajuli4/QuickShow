...
const getSeatType = (seat) => {
    ...
    if (row === "I" || row === "J") return "VIP";
    if (["E", "F", "G", "H"].includes(row)) return "LOVE";
    return "STANDARD";
};
...
const getSeatTypeLabel = (seat, show) => {
    ...
    if (type === "VIP") return "VIP / Luxury";
    if (type === "LOVE") return "Love / Couple";
    return "Standard";
};
...
const getSeatingZone = (seat) => {
    ...
    if (["A", "B", "C", "D"].includes(row)) return "FRONT";
    if (["E", "F", "G", "H"].includes(row)) return "MIDDLE";
    if (row === "I" || row === "J") return "BACK";
    return "";
};
...
const getSeatPrice = (seat, show) => {
    const type = getSeatTypeFromShow(seat, show);
    if (show?.seatPrices) {
        if (type === "VIP") return Number(show.seatPrices.vip ?? SEAT_PRICES.VIP);
        if (type === "LOVE") return Number(show.seatPrices.love ?? SEAT_PRICES.LOVE);
        return Number(show.seatPrices.standard ?? show.showPrice ?? SEAT_PRICES.STANDARD);
    }
    if (type === "VIP") return SEAT_PRICES.VIP;
    if (type === "LOVE") return SEAT_PRICES.LOVE;
    return Number(show?.showPrice || SEAT_PRICES.STANDARD);
};

const getShowPrices = (show) => ({
    standard: Number(show?.seatPrices?.standard ?? show?.showPrice ?? SEAT_PRICES.STANDARD),
    love: Number(show?.seatPrices?.love ?? SEAT_PRICES.LOVE),
    vip: Number(show?.seatPrices?.vip ?? SEAT_PRICES.VIP),
});
...
const SeatLayout = () => {
    ...
    //   1. Array of seat labels: ["A1", "A2"]
    //   2. Object of seat→userId: { "A1": "userId1" }
    //   3. Nested: { occupiedSeats: [...] } or { seats: [...] }
    // =====================================================

    const getOccupiedSeats = async (showId) => {
        try {
            if (!showId) {
                console.warn("getOccupiedSeats: no showId provided");
                return;
            }
            ...
            if (rawData?.success === true || rawData?.success === undefined) {
                // Try multiple paths to find the seats
                const candidates = [
            ...
            // Normalize: trim strings, filter empty
            seats = seats
                .map((s) => String(s).trim())
                .filter(Boolean);

            console.log("✅ Parsed occupied seats:", seats);
    ...
    const totalAmount = selectedTime
        ? selectedSeats.reduce(
              (total, seat) => total + getSeatPrice(seat, selectedTime),
              0
          )
        : 0;
    ...
    const currentPrices = getShowPrices(selectedTime);
                            ...
                            <div className="border border-gray-800 rounded-xl bg-gray-900/60 p-5 lg:sticky lg:top-24">
                                ...
                                <div className="space-y-3">
                                    {shows.map((show) => {
                                        ...
                                        const showPrices = getShowPrices(show);
                                        ...
                                        <div className="flex items-center gap-4">
                                            {availableTypes.map((type) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <span
                                                            className={`w-4 h-4 rounded-full ${config.color} border ${config.border}`}
                                                        />
